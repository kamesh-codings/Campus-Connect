  ...
  return (
    <div className="min-h-screen flex items-center justify-center gradient-mesh p-4 sm:p-6">
      <div className="w-full max-w-lg mx-auto">
        ...
        <div className="glass-card p-7 shadow-2xl animate-fade-in-up">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input name="name" value={formData.name} onChange={handleChange} placeholder="John Doe" className="input-field pl-10 text-sm" required />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-1.5">Student ID</label>
                <input name="studentId" value={formData.studentId} onChange={handleChange} placeholder="CS21089" className="input-field text-sm" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input name="email" type="email" value={formData.email} onChange={handleChange} placeholder="you@campus.edu" className="input-field pl-10 text-sm" required />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select name="department" value={formData.department} onChange={handleChange} className="input-field text-sm">
                  {departments.map((d) => (
                    <option key={d} value={d} className="bg-surface-dark text-white">{d}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-1.5">Year of Study</label>
                <select name="yearOfStudy" value={formData.yearOfStudy} onChange={handleChange} className="input-field text-sm">
                  {[1, 2, 3, 4].map((y) => (
                    <option key={y} value={y} className="bg-surface-dark text-white">Year {y}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input name="password" type="password" value={formData.password} onChange={handleChange} placeholder="••••••" className="input-field pl-10 text-sm" required minLength={6} />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
                  <input name="confirmPassword" type="password" value={formData.confirmPassword} onChange={handleChange} placeholder="••••••" className="input-field pl-10 text-sm" required />
                </div>
              </div>
            </div>
